...
static int process_i8_rv1106(int8_t*             box_tensor,
                             int32_t             box_zp,
                             float               box_scale,
                             int8_t*             score_tensor,
                             int32_t             score_zp,
...
int post_process(rknn_app_context_t* app_ctx, void* outputs, detect_result_group_t* od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant)
        {
            validCount += process_u8((uint8_t*) _outputs[box_idx].buf,
    ...
    {
        nms(validCount, filterBoxes, classId, indexArray, c, app_ctx->nms_thresh);
    }
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        if (id < app_ctx->labels.size())
        {
            strncpy(od_results->results[last_count].name, app_ctx->labels[id].c_str(), OBJ_NAME_MAX_SIZE);
        }
        else
        {
            strncpy(od_results->results[last_count].name, "unknown", OBJ_NAME_MAX_SIZE);
        }
