...
extern "C"
{
    ...
    extern int32_t
    push_buff_in_decMedia_channel(uint32_t u32ChannelId, uint8_t* pData, VDEC_CHN_FORMAT_E vFmt, uint32_t dataLen, uint8_t isEOS, uint32_t fps);
    ...
    typedef struct
    {
        ...
        int32_t vi_len;
        /* fps_out:输出帧率 */
        int32_t fps_out_flex;  // (0:[固定的输出帧率, 输出帧率=(fps_out_num/fps_out_den)],
                               // 1:[可变的输出帧率,完成帧编码后马上输出])
        int32_t fps_out_num;   // (0:[使用默认值], n:[使用n值]) -- 默认值为30
        int32_t fps_out_den;   // (0:[使用默认值], n:[使用n值]) -- 默认值为1
        /* fps_in :输入帧率 */
        int32_t fps_in_flex;  // (0:[固定的输入帧率, 输入帧率=(fps_in_num/fps_in_den)],
                              // 1:[可变的输入帧率])
        ...
        int32_t bps_target;  // 目标码率(0:[根据width、height、fps_out_num、fps_out_den自动计算值],
                             // n:[使用n值])
